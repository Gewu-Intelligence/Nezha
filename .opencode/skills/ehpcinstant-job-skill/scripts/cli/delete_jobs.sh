...
# 验证作业ID格式
if [[ ! "$JOB_ID" =~ ^job-[a-zA-Z0-9]+$ ]]; then
    echo -e "${RED}❌ 错误: 作业ID格式无效${NC}"
    echo -e "${YELLOW}正确格式: job-后跟字母数字字符${NC}"
    exit 1
fi
...
# 执行删除
if $ALIYUN_CLI ehpcinstant DeleteJobs --JobSpec "$JOB_SPEC_JSON" --region "$REGION"; then
